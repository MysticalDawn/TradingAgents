...
def route_to_vendor(method: str, *args, **kwargs):
    ...
    debug = bool(
        config.get("debug")
        or config.get("dataflows_debug")
        or os.getenv("TRADINGAGENTS_DATAFLOWS_DEBUG") == "1"
    )
    ...
    for vendor in fallback_vendors:
        if vendor not in VENDOR_METHODS[method]:
            if vendor in primary_vendors and debug:
                logger.info(
        ...
        for impl_func, vendor_name in vendor_methods:
            try:
                ...
                if vendor == "alpha_vantage":
                    logger.warning(
                        "Alpha Vantage rate limit exceeded, falling back to next available vendor: %s",
                        e,
                    )
        ...
        if vendor_results:
            ...
            if debug:
                logger.debug(
                    "Vendor '%s' succeeded - got %s result(s)",
                    vendor,
                    len(vendor_results),
                )
            ...
            if debug:
                logger.debug("Vendor '%s' produced no results", vendor)
